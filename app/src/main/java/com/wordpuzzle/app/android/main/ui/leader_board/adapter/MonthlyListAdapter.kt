...
import com.wordpuzzle.app.android.databinding.RowMonthlyLayoutBinding
...
class MonthlyListAdapter(
    private var list: List<MonthlyListModel>
) : RecyclerView.Adapter<MonthlyListAdapter.ViewHolder>() {
    private lateinit var listener: RecyclerViewItemClickListener
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = DataBindingUtil.inflate<RowMonthlyLayoutBinding>(LayoutInflater.from(parent.context), R.layout.row_monthly_layout, parent, false)
...
//        return if (list.size >= 4) list.size - 3 else 0
    ...
    inner class ViewHolder(val itemViewHolder: RowMonthlyLayoutBinding) :
